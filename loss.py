...
#         return torch.nn.MSELoss(output,target,reduction='mean')
...
class VOCriterion:
    def __init__(self, t_crit='rms', rot_crit='quat_product', flow_crit='mse', target_t_crit='none',
...
# class EPELoss(F):
#     def __init__(self):
#         assert isinstance(super(EPELoss, self).__init__, object)
...
#         grad = torch.autograd.Variable(x * df_stacked.rsqrt() / (x.nelement() / 2.0))
#         return grad * grad_output, None


class Attack_Criterion(VOCriterion):
    def __init__(self, t_crit='rms', rot_crit='quat_product', flow_crit="", target_t_crit='none',
                 t_factor=0.85, rot_factor=0.8, flow_factor=0.8, target_t_factor=1.0):
        super(Attack_Criterion, self).__init__(t_crit, rot_crit, flow_crit, target_t_crit, t_factor, rot_factor, flow_factor, target_t_factor)
        self.calc_rot_crit = self.calc_rot_quat_product
        self.calc_flow_crit = CalcCriterion(MSELoss)
        self.t_factor=0.85
        self.rot_factor=0.8
        self.flow_factor=0.8
        self.target_t_factor=1.0
